...
function record
  ...
  if string match -qr 'v|video|no audio' $argv
    ...
    if not [ -z "$argv[2]" ]
      ...
      cmd ffmpeg  -t 44 -i $filename.mkv -vf "fps=10,scale=$quality:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 $filename.gif;rm -rf $filename.mkv
  else
